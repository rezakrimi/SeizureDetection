...
# Delta, Theta, Alpha, SMR, Beta, and Gamma waves
# band_widths = [(.5, 3), (4, 7), (8, 12), (12.5, 15.5), (16, 31), (32, 100)]
band_widths = [(0, 3), (3, 6), (6, 11), (11, 20), (20, 30), (30, 40)]
...
def check_seizures(file):
    if file in seizures:
        return True, seizures[file]
    else:
        return False, -1


def patient_channels(file):
    if 'chb24' in file: return ['CZ-PZ', 'FZ-CZ', 'F4-C4', 'C4-P4', 'C3-P3']
    ...
    if 'chb20' in file: return ['P4-O2', 'T8-P8-1', 'C4-P4', 'F4-C4', 'F8-T8']
    if 'chb21' in file: return ['CZ-PZ', 'C3-P3', 'P7-O1', 'P3-O1', 'F7-T7']
    if 'chb22' in file: return ['P3-O1', 'FZ-CZ', 'CZ-PZ', 'C4-P4', 'C3-P3']
...
def smooth_plv(data, w=31, method='Hull'):
    data = data.reshape((-1))
    if method == 'hull':
        res = hullMA(data, w)
    elif method == 'ma':
        ...
        res = savgol_filter(data, w, 3)
    res = res.reshape((-1, 1))
    return res


# returns the energy of each bandwidth for every 't' seconds of input signals in 'raw'
# 'raw' is of type BaseRaw in MNE
def band_width_energy(raw, t, file):
    recording_length = raw.n_times / fs

    #
    # for time in range(10):
    #     t_second_recording = raw.copy().crop(time*t, time*(t+1))
    #     features = []
    #     for band in band_widths:
    #         signal = t_second_recording.copy().filter(band[0], band[1])
    #         for channel in patient_channels():
    #             channel_signal = signal.get_data()[channel]
    #             features.append(sum(channel_signal))
    #     result.append(features)
    # return result

    if (recording_length % t) != 0:
        #raw.crop(0, floor(recording_length / t) * t)
        recording_length = floor(recording_length / t) * t

    print(int(recording_length/t))
    recording_length = int(recording_length)  # make sure recording_length is integer
    result = np.empty((0, int(recording_length/t)))
    print(result.shape)
    raw.pick_channels(patient_channels(file))
    for band in band_widths:
        filtered_data = np.array(raw.copy().filter(band[0], band[1]).get_data()) # is a [number_of_channels x n_samples] matrix
        if filtered_data.shape != (len(patient_channels(file)), recording_length*256):
            filtered_data = filtered_data[:, 0:recording_length*256]
        filtered_data = filtered_data ** 2
        # filtered_data = np.abs(filtered_data)

        #
        filtered_data = filtered_data.reshape((filtered_data.shape[0], int(recording_length/t), -1))
        filtered_data = np.sum(filtered_data, axis=2)
        result = np.concatenate((result, filtered_data))

    # calculating PLV for all possible channel pairs
    channels_index = [i for i in range(0, len(patient_channels(file)))]
    comb = itertools.combinations(channels_index, 2)
    # narrow-band filter for PLV
    data = np.array(raw.copy().filter(18, 20).get_data())
    for c in [(0, 1), (0, 2), (0, 3), (0, 4), (1, 2), (1, 3), (1, 4), (2, 3), (2, 4), (3, 4)]:
        sig1 = data[c[0]]
        sig2 = data[c[1]]
        if sig1.shape != (recording_length * 256):
            sig1 = sig1[0:recording_length * 256]
            sig2 = sig2[0:recording_length * 256]
        sig1 = sig1.reshape((int(recording_length / t), -1))
        sig2 = sig2.reshape((int(recording_length / t), -1))
        sig1 = hilbert(sig1)
        sig2 = hilbert(sig2)
        sig1 = np.angle(sig1)
        sig2 = np.angle(sig2)
        ...
        # plv = np.abs(np.sum(complex_phase_diff, axis=-1, keepdims=True))
        # plv = normalize(plv, axis=0)
        p1 = np.sum(np.real(complex_phase_diff), axis=-1, keepdims=True) ** 2
        p2 = np.sum(np.imag(complex_phase_diff), axis=-1, keepdims=True) ** 2
        plv = (np.sqrt(p1 + p2)) / (t * fs)
        plv = smooth_plv(plv, w=40, method='ma')
        result = np.concatenate((result, plv.T))

    return result.T


def label(raw, t, file, warning_class=False):
    if file in seizures:
        print('has seizure')
    has_seizure, file_seizures = check_seizures(file)
    recording_length = raw.n_times / fs
    if (recording_length % t) != 0:
        #raw.crop(0, floor(recording_length / t) * t)
        recording_length = floor(recording_length / t) * t
    y = np.zeros((int(recording_length/t), 1))
    if has_seizure:
        for s in file_seizures:
            y[ceil(s[0]/t): floor(s[1]/t)+1, :] = 1
    return y


def hilbert_test(raw):
    data = raw.get_data()
    sig1_hill = sig.hilbert(data[0])
    sig2_hill = sig.hilbert(data[1])
    pdt = (np.inner(sig1_hill, np.conj(sig2_hill)) / (np.sqrt(np.inner(sig1_hill, np.conj(sig1_hill)) * np.inner(sig2_hill, np.conj(sig2_hill)))))
    phase = np.angle(pdt)
    print('hi')


def directory_features(directory, t):
    n_channels = 5
    # n_channels*len(band_widths)=energy_fetures & 10 for all possible PLV values & 1 for the label
    result = np.empty((0, n_channels*len(band_widths)+10+1))
    for i, file in enumerate(os.listdir(directory)):
        print('\n\n*****************************FILE #'+str(i)+'\n\n')
        if file.endswith('.edf'):
            raw = mne.io.read_raw_edf(directory+file, preload=True)
            features = band_width_energy(raw, t, file)
            y = label(raw, t, file)
            labeled_features = np.hstack([features, y])
            result = np.vstack([result, labeled_features])

    return result


# file = "/Users/reza/Desktop/EEG/EEG_Data/Patient5/chb05_01.edf"
# raw = mne.io.read_raw_edf(file, preload=True)
# # test = band_width_energy(raw, 3)
# # data = raw.get_data()[0]
# test2 = hilbert_test(raw)
# print(sum(data-data))
# band_width_energy(raw, 3)
# print('hi')


feature = directory_features('/Users/reza/Downloads/temp/', 2)
savemat('patient20_ma_40w'
        '_2s_1channel'
        '.mat', mdict={'data': feature})
print(feature.shape)